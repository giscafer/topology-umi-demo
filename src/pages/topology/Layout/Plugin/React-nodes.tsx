...
import type { Node } from '@topology/core';
import { createDiv, Line, rectangle, s8 } from '@topology/core';
...
const reactNodes =
  (ReactComponent: any) => (ctx: CanvasRenderingContext2D, node: Node) => {
    // 绘制一个底图，类似于占位符。
    rectangle(ctx, node);
    ...
    if (!node.elementLoaded) {
      ...
      if (node && node.data && node.data.props) {
        ...
        if (node.events.length > 0) {
          node.events.forEach((element: any) => {
            // eslint-disable-next-line no-new-func
            const _fn = new Function('params', element.value);
            eventProps[element.name] = async (componentValue: any) => {
              /**
              ...
              if (['datePicker'].includes(node.name)) {
                Store.set(
                  `componentValue-${node.id}`,
                  moment(componentValue).format('YYYY-MM-DD')
                );
              ...
              if (node.data.bind && node.data.bind.length > 0) {
                const _pens = canvas.data.pens;
                function handleNode(b: any) {
                  return new Promise(async (resolve, reject) => {
                    try {
                      const _pen = _pens.find(
                        (pen: { id: any }) => pen.id === b
                      );
                      const idx = _pens.findIndex(
                        (pen: { id: any }) => pen.id === b
                      );
                      const { api, type, paramsArr, handleResult } =
                        _pen.data.http;
                      const queryData = {};
                      paramsArr.forEach(
                        (item: {
                      ...
                      if (_pen.data.echarts) {
                        ...
                        if (['table'].includes(_pen.name)) {
                          _pens[idx].data.props.dataSource = get(
                            data,
                            handleResult,
                            data
                          );
                ...
                Promise.all(
                  node.data.bind.map((item: any) => handleNode(item))
                ).then(() => {
